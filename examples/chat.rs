...
//! and subscribes, and publishes messages across channels
//!
...
pub async fn main() -> Result<()> {
    // Start by reading the username to then use to identify as the author
    // when publishing it
    ...
    // we need dedicated connection for the subscriber as into_subscriber consumes the client
    let addr = "127.0.0.1:6379";
    let mut client = client::connect(addr).await?;
    let mut subscriber = client::connect(addr).await?.into_subscriber();

    // Subscribe the chat channel, it's also the channel where client will publish
    // messages read from user input
    subscriber.subscribe(vec!["chat".into()]).await?;
    println!("connected!");
